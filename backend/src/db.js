...
// ── Connection ─────────────────────────────────────────────────────────────
// On Render  → DATABASE_URL is set as an environment variable (Internal URL)
// Locally    → uses individual DB_* variables from .env (docker-compose)
const pool = new Pool(
  process.env.DATABASE_URL
    ...
    : {
        ...
        password: process.env.DB_PASSWORD || 'NaijaMarket2024!',
        database: process.env.DB_NAME     || 'naijamarketdb',
        ssl:      false,                       // no SSL needed locally
      }
);
